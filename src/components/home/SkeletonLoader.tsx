...
export default function SkeletonLoader({ type }: SkeletonLoaderProps) {
  if (type === 'movie') {
    return (
      <div className="animate-pulse">
        ...
        <div className="mt-2.5 h-3.5 bg-muted/30 rounded-sm" />
        <div className="mt-1.5 h-3 bg-muted/30 rounded-sm w-3/4" />
      </div>
    );
  }
  ...
  return (
    <div className="animate-pulse text-center">
      ...
      <div className="mt-2.5 h-3 bg-muted/30 rounded-sm" />
      <div className="mt-1 h-2.5 bg-muted/30 rounded-sm w-3/4 mx-auto" />
    </div>
  );
}
