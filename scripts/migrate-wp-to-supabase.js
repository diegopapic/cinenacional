...
// VERSIÓN COMPLETA CON MIGRACIÓN DE ESTADOS
...
// NUEVO: Mapeo de estados de WordPress a valores de Supabase
const WP_STATUS_MAPPING = {
    'Completa': 'completed',           // 10,034 películas
    'Inédita': 'unreleased',          // 161 películas
    'En postproducción': 'post_production', // 107 películas
    'En desarrollo': 'in_development',     // 86 películas
    'En preproducción': 'pre_production',  // 68 películas
    'En producción': 'in_production',      // 54 películas
    'Inconclusa': 'unfinished',           // 54 películas
    'En rodaje': 'filming'                 // 22 películas
};
...
const countryIdMap = {
  ...
  "11229": { name: "Argentina", slug: "argentina", code: "AR" } // Agregamos Argentina
};
...
// Logger mejorado con soporte para clasificaciones, coproducción y estados
class MigrationLogger {
    constructor() {
        ...
        Object.values(WP_STATUS_MAPPING).forEach(status => {
            this.logs.productionStatus.distribution[status] = 0;
        });
    }
    ...
    // NUEVO: Log para estados de producción
    logProductionStatus(movieTitle, wpStatus, mappedStatus) {
        if (mappedStatus) {
            this.logs.productionStatus.mapped++;
            this.logs.productionStatus.distribution[mappedStatus]++;
        } else if (!wpStatus) {
            this.logs.productionStatus.null++;
        } else {
            this.logs.productionStatus.notFound++;
            console.warn(`⚠️  Estado no mapeado: "${wpStatus}" para "${movieTitle}"`);
        }
    }
...
async function migrateComplete() {
    ...
    try {
        ...
        // ACTUALIZADO: Obtener películas con campo estado
      ...
      SELECT 
        ...
        -- NUEVO: Campo estado
        pm_estado.meta_value as estado_meta,
      ...
      -- NUEVO: Join para estado
      LEFT JOIN wp_postmeta pm_estado 
        ON p.ID = pm_estado.post_id AND pm_estado.meta_key = 'estado'
      -- Color taxonomy
    ...
    `);
        ...
        for (const [wpId, movie] of moviesMap) {
            try {
                ...
                // NUEVO: Mapear estado de producción
                let productionStatus = null;
                if (movie.estado_meta) {
                    const mappedStatus = WP_STATUS_MAPPING[movie.estado_meta];
                    if (mappedStatus) {
                        productionStatus = mappedStatus;
                        logger.logProductionStatus(cleanedTitle, movie.estado_meta, mappedStatus);
                    } else {
                        console.warn(`⚠️  Estado no mapeado: "${movie.estado_meta}" para "${cleanedTitle}"`);
                        logger.logProductionStatus(cleanedTitle, movie.estado_meta, null);
                    }
                } else {
                    // Si no hay estado, por defecto usar 'completed' ya que la mayoría están completas
                    productionStatus = 'completed';
                    logger.logProductionStatus(cleanedTitle, null, 'completed');
                }
                ...
                console.log(`   - Estado: ${movie.estado_meta || 'Completa (por defecto)'} → ${productionStatus}`);
                ...
                console.log(`   - Países: Argentina (principal)${countryNames.length > 0 ? ', ' + countryNames.join(', ') + ' (coproductores)' : ''}`);
        ...
        // Resumen actualizado con estados
        ...
        // NUEVO: Resumen de estados
        console.log('\n📊 Estados de Producción:');
        console.log(`   - Mapeados correctamente: ${logger.logs.productionStatus.mapped}`);
        console.log(`   - Sin estado (asignados como 'completed'): ${logger.logs.productionStatus.null}`);
        console.log(`   - No mapeados: ${logger.logs.productionStatus.notFound}`);
        console.log('\n   Distribución:');
        Object.entries(logger.logs.productionStatus.distribution).forEach(([status, count]) => {
            if (count > 0) {
                const wpStatus = Object.entries(WP_STATUS_MAPPING).find(([wp, mapped]) => mapped === status)?.[0];
                console.log(`     • ${status} (${wpStatus}): ${count} películas`);
            }
        });
        
        console.log('\n🌍 Coproducción:');
        console.log(`   - Total películas con coproducción: ${logger.logs.coproduction.total}`);
        ...
        logger.logs.summary = {
            ...
            productionStatus: logger.logs.productionStatus,
...
if (args.includes('--help')) {
    console.log(`
Migración Completa WordPress → Supabase (con Estados de Producción)
====================================================================

Uso: node migrate-wp-to-supabase-complete.js [opciones]

Opciones:
  --migrate             Ejecutar migración completa (incluye estados)
  --clean               Limpiar tablas antes de migrar
  --verify-status       Verificar mapeo de estados de producción
...
Mapeo de Estados:
  - Completa → completed
  - Inédita → unreleased
  - En postproducción → post_production
  - En desarrollo → in_development
  - En preproducción → pre_production
  - En producción → in_production
  - Inconclusa → unfinished
  - En rodaje → filming

Nota: Las películas sin estado se asignan como 'completed' por defecto.
      La migración incluye:
      1. Clasificaciones (ratings)
      2. Películas con todos sus metadatos
      3. Estados de producción
      4. Colores y sonido
      5. Países y coproducción
      
      Argentina se agrega automáticamente como país principal para todas las películas.
`);
} else if (args.includes('--verify-status')) {
