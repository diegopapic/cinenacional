...
async function prepareDatesMigration() {
  ...
  try {
    ...
    // 1. Primero obtener estadísticas rápidas
    console.log('📊 1. OBTENIENDO ESTADÍSTICAS RÁPIDAS\n');
    
    const [totalCount] = await wpConnection.execute(`
    ...
    console.log(`Total personas publicadas: ${totalCount[0].total}`);
    
    // Contar personas con fechas (más rápido)
    const [withDatesCount] = await wpConnection.execute(`
      SELECT 
        COUNT(DISTINCT CASE WHEN pm.meta_key = 'fecha_nacimiento' AND pm.meta_value != '' THEN pm.post_id END) as con_nacimiento,
        COUNT(DISTINCT CASE WHEN pm.meta_key = 'fecha_muerte' AND pm.meta_value != '' THEN pm.post_id END) as con_muerte
      FROM wp_postmeta pm
      JOIN wp_posts p ON pm.post_id = p.ID
      WHERE p.post_type = 'persona'
      AND p.post_status = 'publish'
      AND pm.meta_key IN ('fecha_nacimiento', 'fecha_muerte')
    `);
    
    console.log(`Personas con fecha de nacimiento: ${withDatesCount[0].con_nacimiento}`);
    console.log(`Personas con fecha de muerte: ${withDatesCount[0].con_muerte}`);
    
    // 2. Procesar en lotes más pequeños
    console.log('\n📊 2. EXTRAYENDO DATOS EN LOTES\n');
    
    const batchSize = 500; // Procesar de a 500
    const totalBatches = Math.ceil(totalCount[0].total / batchSize);
    let allPeopleWithDates = [];
    
    for (let batch = 0; batch < totalBatches; batch++) {
      const offset = batch * batchSize;
      console.log(`Procesando lote ${batch + 1}/${totalBatches} (${offset + 1}-${Math.min(offset + batchSize, totalCount[0].total)})...`);
      
      const [batchData] = await wpConnection.execute(`
        SELECT 
          p.ID as wp_id,
          p.post_name as slug,
      ...
      allPeopleWithDates = allPeopleWithDates.concat(batchData);
      
      // Mostrar progreso cada 5 lotes
      if ((batch + 1) % 5 === 0) {
        console.log(`   ✅ ${allPeopleWithDates.length} personas procesadas hasta ahora...`);
      }
    }
    
    const peopleWithDates = allPeopleWithDates;
