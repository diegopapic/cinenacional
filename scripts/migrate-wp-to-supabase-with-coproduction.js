// migrate-wp-to-supabase-with-coproduction.js
...
// Función para migrar países y coproducción (YA NO SE USA - se hace inline)
// La mantengo comentada por si quieres referenciarla
/*
async function migrateCountriesAndCoproduction(connection, supabase, movieIdMap) {
    // Esta función ya no se usa porque la coproducción se migra
    // directamente al crear cada película
}
*/
    ...
    `);
        ...
        for (const [wpId, movie] of moviesMap) {
            try {
                ...
                // Guardar el mapeo WP ID -> Supabase ID (ya no es necesario para coproducción)
        ...
        // Ya no necesitamos migrar coproducción por separado porque se hace durante cada película
...
Uso: node migrate-wp-to-supabase-with-coproduction.js [opciones]
...
`);
    ...
    console.log('Para ejecutar la migración completa: node migrate-wp-to-supabase-with-coproduction.js --migrate');
}